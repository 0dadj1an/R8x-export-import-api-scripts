...
doshowhelp () {
    ...
    echo
    echo $0' [-?][-v]|[-r]|[-u <admin_name>] [-p <password>]]|[-P <web ssl port>] [-m <server_IP>] [-d <domain>] [-s <session_file_filepath>]| --SO |[-x <export_path>] [-i <import_path>] [-k <delete_path>] [-l <log_path>]'
    echo
...
limit $APICLIObjectLimit offset $currentgroupoffset details-level "full" -s $APICLIsessionfile --format json | $JQ ".objects[] | '"$systemobjectselector"' | .name | @sh"

details-level "full" -s $APICLIsessionfile --format json | $JQ ".objects[] | '"$systemobjectselector"' | .name | @sh"

details-level "full" -s $APICLIsessionfile --format json | $JQ ".objects[] | '"$systemobjectselector"
...
$JQ '.objects[] | '"$systemobjectselector"' | .name | @sh' -r
