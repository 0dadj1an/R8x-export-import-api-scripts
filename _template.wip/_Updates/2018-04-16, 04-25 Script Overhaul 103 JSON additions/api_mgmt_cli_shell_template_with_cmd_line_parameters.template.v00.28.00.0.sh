...
if [ x"$CLIparm_mgmt" != x"" ] ; then
    if [ x"$mgmttarget" != x"" ] ; then
        export mgmttarget=$mgmttarget" -m \"$CLIparm_mgmt\""
    else
       export  mgmttarget="-m \"$CLIparm_mgmt\""
    fi
else
    export mgmttarget=$mgmttarget
fi

#   Handle domain parameter for mgmt_cli parms
if [ x"$CLIparm_domain" != x"" ] ; then
    if [ x"$domaintarget" != x"" ] ; then
        export domaintarget=$domaintarget" domain \"$CLIparm_domain\""
    else
        export domaintarget="domain \"$CLIparm_domain\""
    fi
else
    export domaintarget=$domaintarget
fi
